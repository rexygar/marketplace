<div class="col-span-12 grid grid-cols-7 _Gastronomia py-20 px-36  sm:px-10  md:px-16 lg:px-24 2xl:px-36 xl:px-36 ">
  <div class="col-span-7 grid md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-3 2xl:grid-cols-3 grid-cols-2 sm:gap-1  gap-6 lg:gap-4">
    ...
    <?php
    $img_producto = null;
    $img_tienda = $tiendas_g->logo;

    if ( $tiendas_g->producto != null) {

    foreach ($tiendas_g->producto as $p) {
    if ($loop->first) {
    $img_producto = $p->img;
